...
app.controller('sellerController' ,function($scope,$controller   ,sellerService){	
	...
	$scope.findByPagination=function(page,rows){
		sellerService.findByPagination(page,rows).success(
			function(response){
	...
	$scope.findById=function(id){
		sellerService.findById(id).success(
			function(response){
	...
	$scope.findWithConditionsByPagination=function(page,rows){
		sellerService.findWithConditionsByPagination(page,rows,$scope.searchEntity).success(
			function(response){
				$scope.list=response.rows;	
				$scope.paginationConf.totalItems=response.total;//更新总记录数
			}			
