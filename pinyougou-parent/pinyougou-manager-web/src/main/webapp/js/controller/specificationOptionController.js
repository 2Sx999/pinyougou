...
app.controller('specificationOptionController' ,function($scope,$controller   ,specificationOptionService){	
	...
	$scope.findByPagination=function(page,rows){
		specificationOptionService.findByPagination(page,rows).success(
			function(response){
	...
	$scope.findById=function(id){
		specificationOptionService.findById(id).success(
			function(response){
	...
	$scope.findWithConditionsByPagination=function(page,rows){
		specificationOptionService.findWithConditionsByPagination(page,rows,$scope.searchEntity).success(
			function(response){
				$scope.list=response.rows;	
				$scope.paginationConf.totalItems=response.total;//更新总记录数
			}			
