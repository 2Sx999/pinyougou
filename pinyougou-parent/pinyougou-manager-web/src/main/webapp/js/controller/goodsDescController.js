...
app.controller('goodsDescController' ,function($scope,$controller   ,goodsDescService){	
	...
	$scope.findByPagination=function(page,rows){
		goodsDescService.findByPagination(page,rows).success(
			function(response){
	...
	$scope.findById=function(id){
		goodsDescService.findById(id).success(
			function(response){
	...
	$scope.findWithConditionsByPagination=function(page,rows){
		goodsDescService.findWithConditionsByPagination(page,rows,$scope.searchEntity).success(
			function(response){
				$scope.list=response.rows;	
				$scope.paginationConf.totalItems=response.total;//更新总记录数
			}			
